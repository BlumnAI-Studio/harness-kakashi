...
- **동명이술 해소됨 (2026-09-04)**: 과거 배포판에 있던 사용자용 `harness-build` 스킬(하네스 빌더)은 제거되어 `harness-kakashi-creator`의 Mode B(에이전트 설계·영입)와 Mode E(구조 검증/버전 관리)로 통합됐다. 배포판은 이제 2스킬 체계(creator/차크라)다. 사용자용 빌더를 배포판에 다시 만들지 말 것 — 기능 추가는 creator에 한다.
