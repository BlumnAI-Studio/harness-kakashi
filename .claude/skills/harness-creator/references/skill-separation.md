...
| 질문 | 예 → | 아니오 → |
|------|------|----------|
| 다른 프로젝트에서도 쓸 수 있는가? | 표준 (harness-creator) | 프로젝트 전용 |
...
| ~~트리거~~ | ~~`harness-creator` SKILL.md~~ | ❌ 표준 스킬 오염 |
...
`harness-creator`의 "프로젝트별 트리거 자동 발견"
...
단 표준 SKILL.md의 수정은 배포 저장소(harness-kakashi)의 릴리스 절차(README "스킬 개발에 참여하기")를 통해서만 이루어진다 —
