...
| 인물 | 능력 | 부르는 대상 |
|------|------|-----------|
| 정원지기 (tamer) | **접목(接木)** — 다른 정원의 스킬을 옮겨 심기 | 다른 닌자의 기술(術) |
| 나루토 (사용자) | **두꺼비 소환술** — 현자를 강림 | 과거 거장의 사상(思想) |

두 능력은 충돌하지 않는다 — 정원지기는 정원을 가꾸고, 나루토는 그 정원에 거장을 부른다.
사용자가 `/harness-creator`로 정원지기를 부른 뒤, 같은 세션에서 두꺼비 소환술을 발동하는 것이 **표준 흐름**이다.
